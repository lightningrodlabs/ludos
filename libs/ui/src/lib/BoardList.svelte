<script lang="ts">
  ...
  import { Group, VoteType, DEFAULT_STICKIE_VOTE_TYPES, Board, BoardType, UngroupedId } from './board';
  ...
  let loading = false
  const newBoard = () => {
    editVoteTypes = cloneDeep(DEFAULT_STICKIE_VOTE_TYPES)
    creating = true
  }
  ...
  const addBoard = async (type: BoardType, name: string, story: string, groups: Group[], voteTypes: VoteType[]) => {
    const board = await store.boardList.makeBoard({type, name, story, groups, voteTypes, stickies:[
      {
          id: uuidv1(),
          x: 0,
          y: 0,
  ...
  const updateBoard = (hash: EntryHashB64) => async (_type:BoardType, name: string, story: string, groups: Group[], voteTypes: VoteType[]) => {
    ...
    if (board) {
      ...
      if (state.story != story) {
        console.log("updating ream story to ",story)
        changes.push(
          {
            type: 'set-story',
            story: story
...
<div class='boards'>
    ...
    Players: {$participants.active.length }
    </div>
    ...
    <div class='board-list'>
        ...
        {#each $boardList.boards as board }
          {#if editingBoardHash === board.hash}
            <BoardEditor boardType={boardType} handleSave={updateBoard(board.hash)} handleDelete={archiveBoard(board.hash)} {cancelEdit} story={editStory} text={editName} groups={editGroups} voteTypes={editVoteTypes} />
          {:else}
